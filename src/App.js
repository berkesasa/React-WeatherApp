  ...
  return (
    <div>
      <div className="container mx-auto flex items-center justify-center h-screen">
        <div
          className={`flex flex-col justify-start items-center relative rounded-2xl z-10 bg-white bg-opacity-20 backdrop-blur-md p-5 weatherContainer transition-all duration-500 h-20  ${ cityData.length === 0  ? "h-20" : "h-[500px]"} overflow-hidden w-1/2 lg:w-1/3`}>
          ...
          <div className={`flex flex-col justify-center w-full weatherData transition-all duration-1000 delay-500 overflow-hidden ${cityData.length === 0 ? "h-0" : "h-full"}`}>
            ...
            <div className="flex justify-between !font-bold text-white mt-5">
              <div className="flex space-x-1 items-center">
                <svg className="max-sm:h-5 sm:h-8 stroke-white" viewBox="0 0 1.2 1.2" fill="none"
                ...
                <div>
                  <div className="weatherHumidity">{cityData.main.humidity}</div>
                  <p className="text-xs">Humidity</p>
                </div>
              </div>
              <div className="flex items-center space-x-1">
                <svg className="max-sm:h-5 sm:h-8 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M11 10h-11v-2h11c.552 0 1-.448 1-1s-.448-1-1-1c-.403 0-.747.242-.905.587l-1.749-.956c.499-.965 1.494-1.631 2.654-1.631 3.971 0 3.969 6 0 6zm7 7c0-1.656-1.344-3-3-3h-15v2h15c.552 0 1 .448 1 1s-.448 1-1 1c-.403 0-.747-.242-.905-.587l-1.749.956c.499.965 1.494 1.631 2.654 1.631 1.656 0 3-1.344 3-3zm1.014-7.655c.082-.753.712-1.345 1.486-1.345.827 0 1.5.673 1.5 1.5s-.673 1.5-1.5 1.5h-20.5v2h20.5c1.932 0 3.5-1.568 3.5-3.5s-1.568-3.5-3.5-3.5c-1.624 0-2.977 1.116-3.372 2.617l1.886.728z" /></svg>
                <div>
                  <p className="weatherWind">{cityData.wind.speed}</p>
